...
export async function fetchUserAndFollowsPosts(userId: string, pageNumber = 1, pageSize = 20) {
  connectToDB();
  ...
  // Create a query to fetch the posts that have no parent (top-level threads) (a thread that is not a comment/reply).
  const postsQuery = Thread.find({ parentId: { $in: [null, undefined] }, author: { $in: [user.follows, userId] } })
    .sort({ createdAt: "desc" })
    .skip(skipAmount)
    .limit(pageSize)
    .populate({ // populate creater
